...
class Portfolio:
    ...
    def open_position(self, size, entry_price, entry_timestamp, position_type='long', leverage=1):
    ...
    def update_positions(self, current_price, current_timestamp):
        for position in self.positions:
            ...
            if position.stop_loss_price is not None:
                if (position.position_type == 'long' and current_price <= position.stop_loss_price) or \
                   (position.position_type == 'short' and current_price >= position.stop_loss_price):
                    self.close_position(position, position.stop_loss_price, current_timestamp)
                    continue
            # Sprawdzenie Take Profit
            if position.take_profit_price is not None:
                if (position.position_type == 'long' and current_price >= position.take_profit_price) or \
                   (position.position_type == 'short' and current_price <= position.take_profit_price):
                    self.close_position(position, position.take_profit_price, current_timestamp)
                    continue
            if position.unrealized_profit <= self.simulation.max_loss:
                print(f"CLOSING POSITION: {current_timestamp} BECAUSE OF {position.unrealized_profit}")
                self.close_position(position, current_price, current_timestamp)

    def get_total_profit(self):
        total_profit = sum([position.total_profit for position in self.closed_positions])
        return total_profit
