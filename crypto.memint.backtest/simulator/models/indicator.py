...
class TechnicalIndicator(models.Model):
    ...
    name = models.CharField(max_length=20, choices=INDICATOR_CHOICES, null=True, blank=True)
    parameters = models.JSONField(null=True, blank=True)  # parameters like period lengths, etc.
    ...
    def validate_parameters(self):
        ...
        for param in self.parameters.keys():
            if param not in allowed_params:
                raise ValueError(f"Parameter for indicator {self.name}: {param} is not allowed.")
