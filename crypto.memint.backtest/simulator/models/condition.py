...
class Condition(models.Model):
    ...
    condition_group = models.ForeignKey(
        ConditionGroup, related_name="conditions", on_delete=models.CASCADE, null=True
    )
    left_operand = models.ForeignKey(
        Operand, on_delete=models.CASCADE, related_name="left_conditions", null=True
    )
    operator = models.CharField(max_length=10, choices=CONDITION_OPERATORS)
    right_operand = models.ForeignKey(
        Operand, on_delete=models.CASCADE, related_name="right_conditions", null=True
    )
    logical_operator = models.CharField(
        max_length=4, choices=LOGICAL_OPERATORS, default="AND"
    )
