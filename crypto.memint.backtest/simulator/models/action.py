...
class Action(models.Model):
    ...
    def execute(self, portfolio, data_row):
        ...
        if self.action_type == 'BUY':
            ...
            size = amount if amount is not None else fixed_trade_value * leverage / price
            ...
            positions_long = [
                ...
                if p.symbol == simulation.symbol and not p.is_closed and p.position_type == 'long'
            ]
            if positions_long:
            ...
            if price_value is None and percent_value is None:
                raise ValueError(f"{self.action_type} action requires 'price' or 'percent' parameter")
            positions = [
                p for p in portfolio.positions
            ...
            for position in positions:
                if price_value is not None:
                    ...
                    method = getattr(position, f"set_{self.action_type.lower()}_percent")
