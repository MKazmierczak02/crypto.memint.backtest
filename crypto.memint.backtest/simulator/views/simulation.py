...
def start_simulation(request):
    ...
    if simulation.status != "Ready":
        return Response(
            {
                "status": "error",
                "message": f"Simulation status is {simulation.status}.",
            },
    ...
    return Response(
        {"status": simulation.status, "task_id": task.id}, status=status.HTTP_200_OK
    )
...
def export_simulation_to_csv(request, simulation_id):
    ...
    simulation = get_object_or_404(
        Simulation, id=simulation_id, strategy__user=request.user
    )
    ...
    for condition_group in simulation.strategy.condition_groups.all().order_by("order"):
        writer.writerow(
            [
                f"Condition Group {condition_group.order} - Action: {condition_group.action.action_type}"
            ]
        )
        for condition in condition_group.conditions.all().order_by("order"):
            ...
            left_params_str = (
                ", ".join(f"{k}={v}" for k, v in left_params.items())
                if left_params
                else ""
            )
            ...
            if condition.constant_value is not None:
                ...
                right_params = (
                    condition.indicator_right.parameters
                )  # Assuming this is a dict
                right_params_str = (
                    ", ".join(f"{k}={v}" for k, v in right_params.items())
                    if right_params
                    else ""
                )
